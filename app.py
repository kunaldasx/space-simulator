...
<style>
  *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  .btn-primary {
    ...
    font-size: clamp(.85rem,4vw,1rem); font-weight:700;
    letter-spacing:.12em; padding:16px 32px;
  ...
  .btn-ghost {
    ...
    padding:14px 24px; cursor:pointer; border-radius:8px;
    min-height:52px; -webkit-appearance:none; transition: all .2s;
  }
  ...
  #scene-launch {
    ...
    /* Use flex column so items stack naturally — no absolute positioning for button */
  ...
  /* rocket sits in the middle of remaining space */
  .rocket-area {
    flex: 1;
    display: flex;
  ...
  .shuttle-wrap.launching {
    animation: liftoff 3.2s ease-in forwards;
  }
  @keyframes liftoff {
    0%   { transform:translateX(-50%) translateY(0)      scale(1); }
    20%  { transform:translateX(-50%) translateY(-20px)  scale(1); }
    100% { transform:translateX(-50%) translateY(-120vh) scale(.2); }
  }
  .rocket-nose {
    width:0; height:0;
  ...
  .shuttle-wrap.launching .rocket-flame {
    animation: burnFlame 3.2s ease-in forwards;
  }
  @keyframes burnFlame {
    0%   { border-top:26px solid #ff6600; opacity:1; }
    50%  { border-top:46px solid #ffaa00; opacity:1; }
    100% { border-top:6px  solid #ffdd00; opacity:.2; }
  }
  .smoke-ring {
    position:absolute; bottom:calc(22vh + 12px); left:50%;
    transform:translateX(-50%);
    width:0; height:0; opacity:0; border-radius:50%; pointer-events:none;
  }
  .smoke-ring.active { animation: smokeOut 2.8s ease-out forwards; }
  @keyframes smokeOut {
    0%   { width:8px;   height:8px;   opacity:.7; background:#888; }
    100% { width:200px; height:200px; opacity:0;  background:#333; }
  }

  /* LAUNCH BUTTON — in normal flow, below rocket area */
  .launch-btn-wrap {
    flex-shrink: 0;
    padding: 20px 0 32px;
    z-index: 20;
  ...
  .btn-launch {
    background:transparent; border:2px solid #88aaff; color:#88aaff;
    font-family:'Orbitron',monospace; font-size:clamp(.9rem,4vw,1.1rem);
    font-weight:700; letter-spacing:.18em; padding:16px 44px;
    cursor:pointer; border-radius:6px; min-height:56px; min-width:200px;
    text-transform:uppercase; -webkit-appearance:none; display:block;
    transition: background .2s, box-shadow .2s;
  }
  .btn-launch:hover, .btn-launch:focus {
    background:#88aaff22; box-shadow:0 0 24px #88aaff44; outline:none;
  }
  .btn-launch:active { background:#88aaff33; transform:scale(.97); }
  ...
  .prompt-card {
    ...
    border-radius:18px;
    padding:clamp(22px,5vw,38px) clamp(18px,5vw,40px);
  ...
  .prompt-card h2 {
    font-family:'Orbitron',monospace;
    font-size:clamp(1rem,4.5vw,1.35rem); margin-bottom:10px;
  }
  .prompt-card p {
    color:#aabfe0; font-size:clamp(.8rem,3.3vw,.93rem);
  ...
  .sun {
    ...
    z-index:5; animation:pulse 3s ease-in-out infinite;
  }
  @keyframes pulse {
  ...
  .planet-label {
    ...
    transform:translateX(-50%);
    font-size:clamp(6px,1.5vw,8px); color:#ffffff55;
  ...
  .float-prompt p {
    color:#aabfe0; font-size:clamp(.76rem,3vw,.86rem);
  ...
  .info-card {
    background:linear-gradient(160deg,#0b1030,#050818);
    border:1px solid #88aaff33; border-radius:22px 22px 0 0;
    padding:26px 20px 40px; width:100%; max-width:540px;
...
const planets = [
  ...
  { name:'Earth',   color:'#3a7bd5', relSize:1,   orbitFrac:.28, speed:2.9,  distance:'149.6M km', sizeRel:'1× (home)',   temp:'15°C avg',        moons:'1',   fact:'The only known planet in the universe to harbour life — from deep-sea vents to the highest mountain peaks.' },
